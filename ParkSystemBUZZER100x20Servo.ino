/*  Parking Gate – Final (I2C LCD + HC-SR04 + Servo D6 + LEDs + Buzzer D10 + Counter)
    Ahmed edition – state machine + debouncing + hysteresis
    UNO: SDA=A4, SCL=A5 | LCD I2C addr = 0x27
*/
...
// غيّرها إلى false إذا ليداتك تضيء عند LOW
...
// ---------- Servo Angles ----------
const int  SERVO_CLOSED_ANG    = 100;   // وضع الإغلاق الحالي
const int  SERVO_OPENED_ANG    = 20;    // وضع الفتح الحالي
const int  SERVO_STEP_DELAY    = 6;     // ms لكل خطوة
...
// ---------- Buzzer (لطيف غير مستمر) ----------
void chirp(unsigned int freq, unsigned int ms, unsigned int gap=40) {
  tone(PIN_BUZZ, freq, ms);
  delay(ms + gap);
  noTone(PIN_BUZZ);
}
void beepOpenPattern() { chirp(1500, 80); chirp(1800, 90); }  // بداية الفتح
void beepClosePattern(){ chirp(1000, 160); }                  // قبل الإغلاق
void beepDone()        { chirp(1800, 60);  }                  // اكتمال الحركة
...
// ---------- LCD ----------
void lcdPad(const char* s, uint8_t row){
  lcd.setCursor(0,row); lcd.print(s);
  for (int i=strlen(s); i<16; i++) lcd.print(' ');
}
void lcdLine1(const char* s){ lcdPad(s,0); }
void lcdLine2(const char* s){ lcdPad(s,1); }
void lcdShowCount(){
  char buf[17];
  snprintf(buf, sizeof(buf), "Cars: %lu", carCount);
  lcdLine2(buf);
}

// ---------- Presence logic (debounce + hysteresis) ----------
...
void setup(){
  ...
  lcdShowCount();                     // عرض العداد عند البدء
...
void loop(){
  ...
  switch(state){
    ...
    case S_OPEN: {
      ...
      // بعد مدة فتح دنيا، وبمجرد تأكيد الغياب → ابدأ الإغلاق وعدّ سيارة
      if((millis()-stateT0>=MIN_OPEN_MS) && absenceLatched){
        carCount++;                              // ← عدّ السيارة هنا مرة واحدة
